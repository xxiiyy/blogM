...
public class SysMenu extends Model<SysMenu>{
    ...
    public static List<SysMenu> buildTree(List<SysMenu> menus){
        List<SysMenu> pMenus = new ArrayList<SysMenu>();
        ...
        recursion(pMenus,menus);
        return pMenus;
    }

    public static void recursion(List<SysMenu> pMenus,List<SysMenu> menus){
        for (SysMenu pMenu : pMenus){
            List<SysMenu> tMenu = new ArrayList<SysMenu>();
            for (SysMenu menu : menus){
                if(pMenu.getId().equals(menu.getpId())){
                    tMenu.add(menu);
                }
            }
            menus.removeAll(tMenu);
            recursion(tMenu,menus);
            pMenu.setcMenus(tMenu);
        }
    }
